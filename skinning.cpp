...
void Skinning::applySkinning(const RigidTransform4d * jointSkinTransforms, double * newMeshVertexPositions) const
{
  ...
  for(int i=0; i<numMeshVertices; i++)
  {
    newMeshVertexPositions[3 * i + 0] = 0;
    newMeshVertexPositions[3 * i + 1] = 0;
    newMeshVertexPositions[3 * i + 2] = 0;
    Vec4d p_bar = Vec4d(restMeshVertexPositions[3*i+0], restMeshVertexPositions[3*i+1],restMeshVertexPositions[3*i+2],1);
    for(int j=0; j<numJointsInfluencingEachVertex; j++){
        Vec4d temp = meshSkinningWeights[i*numJointsInfluencingEachVertex + j]
                * jointSkinTransforms[meshSkinningJoints[i*numJointsInfluencingEachVertex + j]] * p_bar;
        newMeshVertexPositions[3 * i + 0] += temp[0] ;
        newMeshVertexPositions[3 * i + 1] += temp[1] ;
        newMeshVertexPositions[3 * i + 2] += temp[2] ;
    }
  }
