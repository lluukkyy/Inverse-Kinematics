...
// globalTransform of a joint is the transformation that converts a point expressed in the joint's local frame of reference,
//      to the world coordinate frame.
// localTransform is the transformation that converts a point expressed in the joint's local frame of reference,
//      to the coordinate frame of its parent joint. Specifically, if xLocal is the homogeneous coordinate of
//      a point expressed in the joint's local frame, and xParent is the homogeneous coordinate of the same
//      point expressed in the frame of the joint's parent, we have:
    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
    ...
    {
       double angle[9], orientation[9];
       euler2Rotation(eulerAngles[i],angle,rotateOrders[i]);
       euler2Rotation(jointOrientationEulerAngles[i],orientation,rotateOrders[i]);
       Mat3d r =  Mat3d(orientation) * Mat3d(angle);
       localTransforms[i] = RigidTransform4d(r, translations[i]);
    }
    ...
    for(int i=1; i<jointUpdateOrder.size(); i++){
        globalTransforms[jointUpdateOrder[i]] =
                globalTransforms[jointParents[jointUpdateOrder[i]]] * localTransforms[jointUpdateOrder[i]];
    }
}
